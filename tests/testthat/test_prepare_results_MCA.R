...
test_that("Variables results are equal", {
  expect_equal(as.vector(signif(mca$var$coord[,1],3)),
               res$vars[res$vars$Type == "Active" & res$vars$Axis == "1", "Coord"])
  expect_equal(as.vector(signif(mca$var$cos2[,2],2)),
               res$vars[res$vars$Type == "Active" & res$vars$Axis == "2", "Cos2"])
  expect_equal(as.vector(signif(mca$var$contrib[,3],3)),
               res$vars[res$vars$Type == "Active" & res$vars$Axis == "3", "Contrib"])
  expect_equal(as.vector(signif(mca$var$v.test[,1],2)),
               res$vars[res$vars$Type == "Active" & res$vars$Axis == "1", "V.test"])
  expect_equal(as.vector(signif(mca$var$v.test[,1],2)),
               res$vars[res$vars$Type == "Active" & res$vars$Axis == "1", "V.test"])
...
test_that("Qualitative supplementary variables results are equal", {
  expect_equal(as.vector(signif(mca$quali.sup$coord[,1],3)),
               res$vars[res$vars$Type == "Supplementary" & 
                          res$vars$Class == "Qualitative" & res$vars$Axis == "1", "Coord"])
  expect_equal(as.vector(signif(mca$quali.sup$cos2[,2],2)),
               res$vars[res$vars$Type == "Supplementary" & 
                          res$vars$Class == "Qualitative" & res$vars$Axis == "2", "Cos2"])
  expect_equal(as.vector(signif(mca$quali.sup$v.test[,3],2)),
               res$vars[res$vars$Type == "Supplementary" & 
