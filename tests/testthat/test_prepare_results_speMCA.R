...
test_that("Variables results are equal", {
  ...
  expect_equal(as.vector(mca$var$eta2[,5]),
...
test_that("Supplementary individuals results are equal", {
  expect_equal(as.vector(round(mca$supi$coord[,4],3)),
...
test_that("Qualitative data are equal", {
  ...
  expect_equal(as.character(res$quali_data$FrenchPop[rownames(res$quali_data) %in% ids]),
