...
- Add more explicit error messages when data from a correspondance analysis had
  only two rows or two columns (#45)
- Upgrade to scatterD3 1.0.2
...
- Fix supplementary variables not displayed in individual plot for speMCA
  (thanks @419kfj)
...
- Add `speMCA_varsup` function and support for supplementary variables in
  `GDAtools::speMCA` results
...
- Change supplementary variables / individuals handling in dudi.\* functions.
  You now have to supply the entire suprow() ou supcol() result instead if
  sub-elements `$cosup` and `$lisup`.
...
- Change points opacity according to contrib or cos2 in PCA/MCA individual plots
  (suggestion by @ginolhac)
- Add ability to hide individuals points labels based on contribution value
  (suggestion by @ginolhac)
...
- Add a "Get R code" button which allows to get the R code to reproduce the
  displayed plot (minus custom labels positions)
...
- Add ability to color individual points according to one of the qualitative
  variables in MCA
- Add control to hide Rows or Columns in CA plot and tables
- Add the ability to change point size and sizes range in variables plots for
  CA and MCA.
